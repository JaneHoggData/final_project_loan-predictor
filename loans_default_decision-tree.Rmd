...
sets <- stratified(loansmodelling, group = c("loan_status", "loan_amnt", "term", "dti"), bothSets = TRUE, size = 0.2)
...
```{r}
loan_fit <- rpart(formula = loan_status ~ ., 
                     data = loan_train, 
                     method = 'dti')
...
```{r}
rf_classifier <- ranger(loan_status ~.,
                        data - loan_train,
                        importance = "dti",
                        num.trees = 1000,
```
