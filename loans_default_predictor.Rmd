...
```{r}
loansgeo <- lending_club_loans %>%
   select(loan_status, emp_length)
ggplot(loansgeo)+
  geom_bar(aes(x = emp_length), fill = "light blue")+
  coord_flip()+
...
ggplot(aes(x = loan_status, y = total_count, fill = emp_length))+
  geom_bar(stat = "identity" , position = "dodge") + 
  scale_fill_brewer(palette="Dark2") + 
  ylab("Total count") + xlab("Loan Status")+
labs( x= "Loan Status",
        y= "Number of Loans",
        title = "Lending Club Status of Loans v Employment Years")
```
...
```{r}
ggplot(default1) +
  geom_point(aes(x=dti, y= ), fill = "light blue")+
   labs( x= "Debt to Income Ratio %",
        y= "Number of Default Loans",
        title = "Lending Club Loans Data",
        subtitle = "DTI %")
...
```{r}
ggplot(default1)+
  aes(loan_amnt, int_rate, colour = term)+
  geom_point()
...
## But need to think carefully about the statistical significance of each predictor we ##add into the model. A good first step in this direction is to examine the p
...
##from this we can see that dti, loan amount and term of the loan are statistically significant but the annual income is not.
